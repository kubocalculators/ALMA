...
# Updates since v2
# - AL Intensity calculations from DLI (strict photoperiod, dimmable intensity)
# - Using hourly intensity needed - LED_dimmable_usage() was added which calculates electrical consumption for variable light intensity
# Updates since v3
# - Dimmable calculation uses iterative method to determine AL intensity needed to reach DLI at the end of photoperiod

# Updates since v4
# - bisection method calculation improved computation efficiency
...
def bisection_method_intensity(day_data, dli_target, photoperiod):
    """
    Computes the daily intensity target iteratively to meet DLI and photoperiod
    using a faster bisection approach with precomputed solar data.

    Key optimization:
    - Avoids recomputing full arrays each iteration
    - Uses sorted solar values + cumulative sums for O(log n) evaluation
    """
    df = day_data.copy()

    # --- Filter ONLY hours where artificial lighting is possible ---
    solar = df.loc[df["AL_Possible"], "Solar_Intensity"].values
    if len(solar) == 0:             # If no hours are allowed to have AL then the daily intensity is 0
        return 0.0

    # --- Pre-sort solar values (done ONCE, not every iteration) ---
    solar_sorted = np.sort(solar)

    # --- Precompute cumulative sum (for fast partial sums) ---
    cumsum = np.cumsum(solar_sorted)

    # Bounds for bisection [umol/m2/s]
    low = 0
    high = 3000

    # Set error tolerance & max iters
    tol = 1e-4                              # mol/m2/d
    max_iters = 25

    # --- Fast DLI calculation using math instead of arrays ---
    def fast_dli(mid):
        """
        Computes DLI without rebuilding arrays.

        Logic:
        - For solar >= mid → use solar
        - For solar < mid → use mid
        """

        idx = np.searchsorted(solar_sorted, mid)                            # Find split point (O(log n))       
        solar_high = cumsum[-1] - (cumsum[idx - 1] if idx > 0 else 0)       # Sum of solar values >= mid

        total = solar_high + idx * mid                                # Total intensity (vector-free)

        # Convert to DLI
        return total * 3600 / 1e6

    # Bisection loop
    for _ in range(max_iters):
        mid = (low + high) / 2

        dli_calculated = fast_dli(mid)

        if abs(dli_target - dli_calculated) < tol:
            return mid

        if dli_calculated < dli_target:
            low = mid
        else:
            high = mid
