...
with st.form("controls", clear_on_submit=False):
    ...
    rad_setpoint = st.number_input("Radiation setpoint (W/m²)", min_value=0.0, value=400.0, step=10.0)
    ...
    screen_1_lower_limit = st.number_input("Screen 1 lower radiation limit (W/m²)", min_value=0.0, value=450.0, step=10.0)
    screen_1_upper_limit = st.number_input("Screen 1 upper radiation limit (W/m²)", min_value=0.0, value=550.0, step=10.0)

    st.subheader("Screen 2")
    screen_2_shading_pct = st.number_input("Screen 2 shading (%)", min_value=0.0, max_value=100.0, value=20.0, step=1.0)
    screen_2_energy_pct = st.number_input("Screen 2 energy saving (%)", min_value=0.0, max_value=100.0, value=47.0, step=1.0)
    screen_2_lower_limit = st.number_input("Screen 2 lower radiation limit (W/m²)", min_value=0.0, value=550.0, step=10.0)
    screen_2_upper_limit = st.number_input("Screen 2 upper radiation limit (W/m²)", min_value=0.0, value=600.0, step=10.0)
    ...
    if system == "LED":
        ...
        gh_temp_setpoint = st.number_input("GH temperature setpoint (°C)", min_value=-30.0, max_value=60.0, value=24.0, step=0.5)
    ...
    try:
        ...
        if use_screen_model:
            ...
            sp = ScreenParams(
                ...
                screen_1_energy_pct=float(screen_1_energy_pct),
                ...
                screen_2_energy_pct=float(screen_2_energy_pct),
