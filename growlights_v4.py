...
def bisection_method_intensity(day_data, dli_target, photoperiod):
    ...
    """
    Computes the daily intensity target iteratively to meet DLI and photoperiod exactly given solar radiation
    day_data is intended to be a subset of the weather dataframe with Solar Intensity (umol/m2/s) for a unique "Year", "Month", "Day" key
    """
    df = day_data.copy()
    df["Hourly_Target"] = 0

    # Set upper and lower bounds for the intensity target
    low = 0                                         # umol/m2/s
    high = 3000                                     # umol/m2/s (very high to ensure initial calculated DLI is higher than DLI target)
    ...
    for _ in range(max_iters):
        ...
        # If AL_Possible is TRUE, use mid as the target for the hour, otherwise the target is 0
        df["Hourly_Target"] = np.where(df["AL_Possible"], mid, 0.0)
        
        # Calculate the solar + lamp intensity, taking the maximum of either the target or the natural intensity
        total_hourly_intensity = np.maximum(df["Hourly_Target"], df["Solar_Intensity"])         # umol/m2/s
        
        # Calculate DLI for current I_target
        dli_calculated = total_hourly_intensity.sum() *3600 /1e6                # mol/m2/h

        # Calculate error against DLI target
